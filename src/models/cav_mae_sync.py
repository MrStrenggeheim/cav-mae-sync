...
class CAVMAE(nn.Module):
    ...
    def __init__(
        ...
        if self.contrastive_heads:
            self.constrative_head_audio = nn.ModuleList(
                [
                    Block(
                        embed_dim,
                        num_heads,
            ...
            self.constrative_head_visual = nn.ModuleList(
                [
                    Block(
                        embed_dim,
                        num_heads,
    ...
    def forward_encoder(self, a, v, mask_ratio_a, mask_ratio_v, mask_mode="unstructured"):
        ...
        x = torch.cat(
            (a[:, 1:] if self.cls_token else a, v[:, 1:] if self.cls_token else v),
            dim=1,
        )
        ...
        if self.num_register_tokens > 0 and self.keep_register_tokens:
            ...
            x = torch.cat(
                (
                    ...
                    x[
                        :,
                        self.patch_embed_a.num_patches + self.num_register_tokens : -self.num_register_tokens,
                        :,
                    ],
                ),
                dim=1,
    ...
    def forward_contrastive(self, audio_rep, video_rep, mode="train", contrast_bidirect=False, contrast_inter_weight=0.4, contrast_intra_weight=0.6):
        ...
        if mode == "train":
            ...
            correct_matches = video_indices[max_indices] == torch.arange(
                total.shape[0] // self.total_frame, device=total.device
    ...
    def forward(
        ...
        if self.multi_ratio_masking:
            ...
            if torch.distributed.is_initialized():
                ...
                seed = torch.tensor(
                    int(torch.empty((), device=audio.device).uniform_().item() * 1e6),
                    device=audio.device,
                )
        ...
        if self.cls_token:
            ...
            # cls_a/cls_v already contain the CLS tokens from forward_encoder (lines 591-592)
            # Include latent_c_a/v means to ensure all computed tensors are used (avoids DDP issues)
            return {
                'loss': loss,
                'loss_mae': loss_mae,
                'loss_mae_a': loss_mae_a,
    ...
    def forward_inpaint(
        ...
        latent, mask_a, ids_restore_a, mask_v, ids_restore_v, latent_c_a, latent_c_v = self.forward_encoder(
...
class CAVMAEFT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        if self.aggregate == "self_attention_cls":
            torch.nn.init.normal_(self.cls_cls_token, std=0.02)
        self.apply(self._init_weights)
