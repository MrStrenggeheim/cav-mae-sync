...
class SimpleAVDataset(Dataset):
    """
    Simplified audio-visual dataset.
    Expects CSV with columns: ['video_name', 'target'].
    Returns per-video tensors:
        audio  -> [T, 1024, 128]
        video  -> [T, 3, 224, 224]
    """

    def __init__(self, csv_file, audio_conf):
        self.df = pd.read_csv(csv_file)
        assert {'video_name', 'target'} <= set(self.df.columns), \
            "CSV must contain columns: 'video_name' and 'target'"
        self.audio_conf = audio_conf
        self.num_samples = len(self.df)
        print(f"Loaded {self.num_samples} samples from {csv_file}")

        # basic settings
        self.melbins = audio_conf.get('num_mel_bins', 128)
        self.target_length = audio_conf.get('target_length', 1024)
        self.norm_mean = audio_conf.get('mean', -5.081)
        self.norm_std = audio_conf.get('std', 4.4849)
        ...
        self.mode = audio_conf.get('mode', 'eval')

        # base dirs (can be overridden in config)
        self.audio_root = audio_conf.get('audio_root', 'audio')
        self.video_root = audio_conf.get('video_root', 'video_frames')
    ...
    def _wav2fbank(self, filename):
        try:
            ...
            print(f"[WARN] Audio load failed: {filename} ({e})")
    ...
    def _get_image(self, path):
        try:
            ...
            return torch.zeros(3, self.im_res, self.im_res)

    def _get_all_frames(self, video_name):
        frame_paths = []
        for i in range(self.total_frame):
            frame_path = os.path.join(self.video_root, video_name, f"frame_{i}", f"{video_name}.jpg")
    ...
    def __getitem__(self, index):
        ...
        # audio path
        audio_path = os.path.join(self.audio_root, f"{video_name}.wav")
        fbank = self._wav2fbank(audio_path)

        # gather frames
        fbanks, images, frame_indices = [], [], []
        frames = self._get_all_frames(video_name)
        if not frames:
            # fallback: dummy frame
            frames = [(None, 0)]

        for fpath, idx in frames:
            fbanks.append(fbank)
            img = self._get_image(fpath) if fpath else torch.zeros(3, self.im_res, self.im_res)
            images.append(img)
            frame_indices.append(idx)

        fbanks = torch.stack(fbanks)           # [T, 1024, 128]
        images = torch.stack(images)           # [T, 3, 224, 224]
        frame_indices = torch.tensor(frame_indices, dtype=torch.long)

        return fbanks, images, label, video_name, frame_indices
...
def eval_collate_fn(batch):
    ...
    fbanks = torch.stack(fbanks)     # [B, T, 1024, 128]
    images = torch.stack(images)     # [B, T, 3, 224, 224]
    labels = torch.stack(labels)
    frame_indices = torch.stack(frame_indices)
    return fbanks, images, labels, list(video_ids), frame_indices
