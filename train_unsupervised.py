...
class CAVMAEModule(pl.LightningModule):
        ...
        if cls_a is not None and cls_v is not None:
            with torch.no_grad():
                ...
                if agg_method == "all":
                    ...
                    # Step-level logging (Trainer's log_every_n_steps handles sampling)
                    self.log("train/sync/mean_step", per_sample_sim.mean(), on_step=True, on_epoch=False, batch_size=len(video_ids))
                    self.log("train/sync/min_step", per_sample_sim.min(), on_step=True, on_epoch=False, batch_size=len(video_ids))
                    self.log("train/sync/p10_step", torch.quantile(per_sample_sim, 0.1), on_step=True, on_epoch=False, batch_size=len(video_ids))
                    self.log("train/sync/p25_step", torch.quantile(per_sample_sim, 0.25), on_step=True, on_epoch=False, batch_size=len(video_ids))
                    self.log("train/sync/variance_step", per_sample_sim.var(), on_step=True, on_epoch=False, batch_size=len(video_ids))
                else:
                    ...
                    # Step-level logging (Trainer's log_every_n_steps handles sampling)
                    self.log("train/sync_score_step", sync_score, on_step=True, on_epoch=False, prog_bar=True, batch_size=len(video_ids))
                    self.log("train/sync/variance_step", per_sample_sim.var(), on_step=True, on_epoch=False, batch_size=len(video_ids))

        # Logging — losses and accuracies
        bs = len(video_ids)
        self.log("train/loss/total", loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=bs)
        self.log("train_loss_total", loss, on_step=True, on_epoch=True, logger=False, batch_size=bs)
        self.log("train/loss/mae", loss_mae, on_step=True, on_epoch=True, prog_bar=True, batch_size=bs)
        self.log("train/loss/contrast", loss_c, on_step=True, on_epoch=True, batch_size=bs)
        self.log("train/acc/intra", intra_acc, on_step=True, on_epoch=True, prog_bar=True, batch_size=bs)
        self.log("train/acc/inter", inter_acc, on_step=True, on_epoch=True, prog_bar=True, batch_size=bs)

        # MAE loss split by modality (diagnose audio vs video reconstruction quality)
        loss_mae_a = outputs.get("loss_mae_a")
        loss_mae_v = outputs.get("loss_mae_v")
        if loss_mae_a is not None:
            self.log("train/loss/mae_audio", loss_mae_a, on_step=True, on_epoch=True, batch_size=bs)
            self.log("train/loss/mae_video", loss_mae_v, on_step=True, on_epoch=True, batch_size=bs)

        # Embedding diagnostics (detect representation collapse)
        if cls_a is not None:
            with torch.no_grad():
                self.log("train/embed/norm_audio", cls_a.norm(dim=-1).mean(), on_step=True, on_epoch=True, batch_size=bs)
                self.log("train/embed/norm_video", cls_v.norm(dim=-1).mean(), on_step=True, on_epoch=True, batch_size=bs)
    ...
    def on_before_optimizer_step(self, optimizer):
        """Log gradient norm for training diagnostics."""
        total_norm = 0.0
